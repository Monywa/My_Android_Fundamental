    ...
    override suspend fun doWork(): Result {
        return withContext(Dispatchers.IO){
            ...
            do{
                ...
                quality-=(quality*1.0).roundToInt()
            }while(
                outputBytes.size > compressionThresholdInBytes && quality > 5
            )

            val file=File(
                appContext.cacheDir,"${params.id}.jpg"
            )
            file.writeBytes(outputBytes)

             Result.success(
                workDataOf(
                    KEY_RESULT_PATH to file.path
                )
            )
        }
    }
